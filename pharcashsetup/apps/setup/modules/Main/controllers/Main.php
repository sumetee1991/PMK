...
class Main extends MY_Controller
{
    ...
    public function update_kiosk_queuelocation($queuelocationuid)
    {
     ...
     // userauth------------------------------------------------------------------------    
     public function userauth($queuelocationuid = NULL)
     {
         $ViewData = array(
             'Site_Title' => 'Userauth Setup',
             'FunctionName' => 'เพิ่มผู้ใช้งาน',
             'Script' => array(
                 'Main' => 'Extension/usercreate/usercreate_script',
             )
         );
         $ViewData = $this->userauth_view($ViewData, $queuelocationuid);
         $this->renderView($ViewData);
     }
     public function userauth_view($Template = NULL, $queuelocationuid = NULL)
     {
         $thisgroup = $this->GroupprocessUID['userauth'];
         $View = 'Extension/usercreate/usercreate_view';
         $returnArray = is_array($Template);
         if (!$returnArray) {
             $Template = array();
         }
         ...
         $Template['Data']['thisQueuelocation'] = $queuelocationuid ? $this->MainMDL->get_row_queuelocation($queuelocationuid) : $this->MainMDL->get_first_queuelocation();
         $Template['Data']['Queuelocation'] = $this->MainMDL->get_queuelocation();
         $Template = $this->view_usercreate_div($Template, $thisgroup, 'Extension');
     ...
     public function userauth_view_location($queuelocationuid = 6)
     {
         $this->userauth_view_write(NULL, $queuelocationuid);
     }
 
     public function userauth_view_write($Template = NULL, $queuelocationuid = NULL)
     {
         $thisgroup = $this->GroupprocessUID['userauth'];
         $View = 'Extension/usercreate/usercreate_view_write';
         $returnArray = is_array($Template);
         if (!$returnArray) {
             $Template = array();
         }
    ...
    // userauthorization------------------------------------------------------------------------    
    public function userauthorization($queuelocationuid = NULL)
    {
        $ViewData = array(
            'Site_Title' => 'Userauthorization Setup',
            'FunctionName' => 'เพิ่มสถานที่ทำงานกับผู้ใช้งาน',
            'Script' => array(
                'Main' => 'Extension/usercontrol/usercontrol_script',
            )
        );
        $ViewData = $this->userauthorization_view($ViewData, $queuelocationuid);
        $this->renderView($ViewData);
    }
    public function userauthorization_view($Template = NULL, $queuelocationuid = NULL)
    {
        $thisgroup = $this->GroupprocessUID['userauthorization'];
        $View = 'Extension/usercontrol/usercontrol_view';
        $returnArray = is_array($Template);
        if (!$returnArray) {
            $Template = array();
        }
    ...
    public function userauthorization_view_location($queuelocationuid = 6)
    {
        $this->userauthorization_view_write(NULL, $queuelocationuid);
    }

    public function userauthorization_view_write($Template = NULL, $queuelocationuid = NULL)
    {
        $thisgroup = $this->GroupprocessUID['userauthorization'];
        $View = 'Extension/usercontrol/usercontrol_view_write';
        $returnArray = is_array($Template);
        if (!$returnArray) {
            $Template = array();
        }
    ...
    public function view_usercreate_div($Template, $groupprocessuid, $ExtVar = 'Ext')
    {
        $this->load->model('UserAuthMDL');
        $this->UserAuthMDL->get_uid_userauth();
        $Template['Data']['AllUser'] = $this->UserAuthMDL->get_userauth();
        $Template['Data']['CurrentAuth'] = $this->UserAuthMDL->get_userauth();
        $Template['Data']['Queuelocation'] = !isset($Template['Data']['Queuelocation']) ? $this->MainMDL->get_queuelocation() : $Template['Data']['Queuelocation'];
        $Template[$ExtVar]['usercreate'] = "$this->ModuleName/Extension/usercreate/usercreatel_view";
        $Template[$ExtVar]['usercreate'] = "$this->ModuleName/Extension/usercreate/usercreatel_view";
        // $Template['Script']['usercontrol'] = 'Extension/usercontrol/usercontrol_div';
        return $Template;
    }
    public function add_usercreate()
    {
        $Input = $this->input->post();
        // $username_main['username'] = $username_input;
        // $Input['uesrname'] = $uesrname;
        $this->load->model('UserAuthMDL');
        return $this->UserAuthMDL->add_userauth($Input);
    }
    public function update_usercreate($uid)
    {
        $Input = $this->input->post();
        $Input['uid'] = $uid;
        $this->load->model('UserAuthMDL');
        // var_dump($Input);
        return $this->UserAuthMDL->update_userauth($Input);
    }
    public function remove_userusercreate($uid)
    {
        $this->load->model('UserAuthMDL');
        return $this->UserAuthMDL->del_userauth(array('uid' => $uid));
    }

    public function view_usercontrol_div($Template, $groupprocessuid, $ExtVar = 'Ext')
    {
        $this->load->model('UserAuthorizationMDL');
        $this->UserAuthorizationMDL->get_uid_userauthorization();
        $Template['Data']['AllUser'] = $this->UserAuthorizationMDL->get_userauthorization();
        $Template['Data']['CurrentAuth'] = $this->UserAuthorizationMDL->get_userauthorization();
        $Template['Data']['Queuelocation'] = !isset($Template['Data']['Queuelocation']) ? $this->MainMDL->get_queuelocation() : $Template['Data']['Queuelocation'];
        $Template[$ExtVar]['usercontrol'] = "$this->ModuleName/Extension/usercontrol/usercontrol_view";
        $Template[$ExtVar]['usercontrol'] = "$this->ModuleName/Extension/usercontrol/usercontrol_view";
        // $Template['Script']['usercontrol'] = 'Extension/usercontrol/usercontrol_div';
        return $Template;
    }
    public function add_usercontrol()
    {
        ...
        // $Input['uesrname'] = $uesrname;
        $this->load->model('UserAuthorizationMDL');
        return $this->UserAuthorizationMDL->add_userauthorization($Input);
    }
    public function update_usercontrol($uid)
    {
        ...
        $this->load->model('UserAuthorizationMDL');
        // var_dump($Input);
        return $this->UserAuthorizationMDL->update_userauthorization($Input);
    }
    public function remove_usercontrol($uid)
    {
        $this->load->model('UserAuthorizationMDL');
        return $this->UserAuthorizationMDL->del_userauthorization(array('uid' => $uid));
    }
    ...
    public function report_ajax()
    {
        ...
        switch ($Input['topic']) {
            ...
            case 2: //ออกคิวยา - MedQueue
                $views['Data_Count'] = $this->ReportMDL->getMedQueueReport_count($Input['queuelocationuid'], $Input['date_from'], $Input['date_to']);
            ...
            case 7: //รวม - Total
                $views['Data_Count'] = $this->ReportMDL->getMedQueueReport_count($Input['queuelocationuid'], $Input['date_from'], $Input['date_to']);
                $views['Data'] = $this->ReportMDL->getTotalReport($Input['queuelocationuid'], $Input['date_from'], $Input['date_to']);
                $output['table'] = "total_report";
                $output['html'] = $this->load->view('Report/ajax/total', $views, TRUE);
                break;
            default:
    ...
    public function get_queuelist_queuelocation($queuelocationuid)
    {
        $this->load->model('MainMDL');
        $query = $this->MainMDL->get_queuelist_queuelocation($queuelocationuid);
        echo json_encode($query);
    }
    /* -- Additional -- */
}
