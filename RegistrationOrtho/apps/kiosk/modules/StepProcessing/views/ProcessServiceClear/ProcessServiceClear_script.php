<script>
	$(document).ready(function() {
       ...
       .done(function(response) {
		...
		function search_patient(key_value) {
    
	
			


			var box_data = key_value;
			var data = {
				idcard: box_data,
					...
					if (data['step'] == "next8scan") {
						...
						$.ajax({ //create session user info
							...
							success: function(msg) {
								...
								group = data['UserInfo'][0]['group'];
								$("#conf_no").val(group);
								$("#conf_no").attr('data-id',session_idcard);
								$("#conf_no").attr('data-hn',session_hn);
								...
								var gender = (data['UserInfo'][0]['infoData']['gender'] == '') ? '-' : data['UserInfo'][0]['infoData']['gender'];
								console.log(gender);
...
gender=2;
								...
								$.ajax({ //create session
									url: UrlBaseCont + '/StepProcessing/scanInput',
									type: "post",
									dataType: "json",
									data:{
										citizenid:idcard,
										prefixname_th:prename,
										firstname_th:forename,
										lastname_th:surname,
										dob:birth,
										gender:gender,
									},
...
var idcard=$(this).attr('data-id');
	...
	function show_list_row(worklistuid, numrow, flowname, closevs, closenewhn, count_patient_newhn, closepayor, count_patient_payor, sendimg, clinicname) {
		...
		if ((closevs != '' || closenewhn != '' || closepayor != '') && closevs != null) {
			var numrow='';
			var sendimg = '<?= single_img('kiosk/resources/images/icon/G1.png', array('class' => '_img_02')); ?>';
			show_result = 'เสร็จสิ้น';
