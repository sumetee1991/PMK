...
class Websocket extends MY_Controller
{
    ...
    public function connectSocket($urlAPP, $portAPP, $urlMID, $portMID, $localurl)
    {
        // $con = array(
        ...
        $con = array(
            'appsocket' => 'io.connect( "http://' . $urlAPP . ':' . $portAPP . '" )',
            'midSocket' => 'io.connect("http://' . $urlMID . ':' . $portMID . '")',
            'baseurlMid' => 'http://' . $urlMID . ':' . $portMID,
    ...
    {
        $Connection = $this->connectSocket(LINKURL, PORTAPP, LINKURL, PORTMID, LOCALURL);
