   ...
   $(document).ready(function() {
      ...
      function show_clinic() {
         $.ajax({
            cache: false,
            type: 'ajax',
            url: ajaxURL + "/clinic_data"+`?_${Date.now()}`,
            async: false,
            dataType: 'json',
            success: function(data) {
               var html = '';
               ...
               for (i = 0; i < data.length; i++) {
                  ...
                  html += '<div class="col-lg-4 col-sm-6 col-12 mb-2"><div class="card  shadow h-100  bg-light"><div class="card-body"><input type="hidden" value="' + j + '"  data-sort="' + data[i].uid + '">' +
                     '<div style="font-size:20px;font-family:cs_prajad_b;font-weight:300;">วัน : ' + data[i].day + '</div>' +
                     '<div style="font-size:20px;font-family:cs_prajad_b;font-weight:300;margin: 10px;">Limit : ' + data[i].limit + '</div>' +
                     '<div>' +
                     ...
                     '<a href="javascript:void(0);" class="btn btn_pmk btn-sm item_edit btn_edit" data-uid="' + data[i].uid + '" data-limit="' + data[i].limit + '" data-day="' + data[i].day + '"><i class="fa fa-edit" aria-hidden="true"></i> แก้ไข</a>' + ' ' +
      ...
      $('#show_data').on('click', '.item_edit', function() {
         ...
         $('#header_txt').html('จำนวนที่จำกัดวัน '+ day + '');
         ...
         $(`.visit_active`).attr("checked", false);
         $(`.active[value="${active}"]`).trigger('click');
         $(`.visit_active[value="${visit_active}"]`).trigger('click');
