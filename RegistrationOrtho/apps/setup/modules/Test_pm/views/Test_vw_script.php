<script>
   ...
   $(document).ready(function() {
      ...
      $("#show_data .counter_list").sortable({
         update: function(event, ui) {
            $('#show_data .counter_list .card .card-body input').each(function(i) {
               var numbering = i + 1;
      ...
      function update_sort($sort, $id) {
         $.ajax({
            url: ajaxURL + "/update_counter_sort/" + $id + '/' + $sort,
            type: "post",
            data: {
               sort: $sort
            },
      ...
      function show_counter() {
         $.ajax({
            cache: false,
            type: 'ajax',
            url: ajaxURL + "/counter_data"+`?_${Date.now()}`,
            async: false,
            dataType: 'json',
            headers: {
               'Access-Control-Allow-Origin': '*',
            ...
            success: function(data) {
               var html = '<div class="counter_list row">';
               var i;
               var j = 1;
               for (i = 0; i < data.length; i++) {
                  html += '<div class="col-lg-4 col-sm-6 col-12 mb-2"><div class="card  shadow h-100  bg-light"><div class="card-body">' + '<input type="hidden" value="' + j + '" data-sort="' + data[i].uid + '">' +
                     '<div style="font-size:20px;font-family:cs_prajad_b;font-weight:300;">' + data[i].countername + '</div>' +
                     '<div>' +
                     '<a href="javascript:void(0);" class="btn btn_pmk btn-sm item_edit btn_edit" data-uid="' + data[i].uid + '" data-product-name="' + data[i].countername + '" data-counterno="' + data[i].counterno + '" data-groupprocess="' + data[i].groupprocessuid + '" data-active="' + data[i].active + '"><i class="fa fa-edit" aria-hidden="true"></i> แก้ไข</a>' + ' ' +
                     '<a href="javascript:void(0);" class="btn btn_pmk btn-sm item_delete" data-product-id="' + data[i].uid + '" style="font-family:Mitr_light;font-size:16px;"><i class="fa fa-times" aria-hidden="true"></i> ลบ</a>' +
                     '</div>' +
                     '</div></div></div>';
                  j++;
               }
      ...
      function show_groupprocess() {
         $.ajax({
            cache: false,
            type: 'ajax',
            url: ajaxURL + "/get_groupprocess"+`?_${Date.now()}`,
            async: false,
            dataType: 'json',
            headers: {
               'Access-Control-Allow-Origin': '*',
      ...
      $('#btn-add').click(function() {
         $('#header_txt').html('เพิ่มช่องบริการ');
      ...
      $('#show_data').on('click', '.item_edit', function() {
         ...
         $('#header_txt').html('แก้ไขช่องบริการ');
         ...
         $(`.active[value="${active}"]`).trigger('click');
      ...
      $('#btn_update').click(function(e) {
         ...
         if ($('#counter_name').val() == "") {
            ...
            // alert(uid);
            $.ajax({
               url: ajaxURL + "/update_counter/" + uid,
               headers: {
                  'Access-Control-Allow-Origin': '*',
                  'Access-Control-Allow-Methods': 'POST, GET, OPTIONS',
                  'Access-Control-Allow-Headers': 'Authorization, Origin, X-Requested-With, Content-Type, Accept',
