   ...
   $(document).ready(function() {
                     ...
                     success: function(data) {
                        ...
                        for (i = 0; i < data.length; i++) {
                           html += '<div class="col-4"><div class="card bg-light shadow mb-2"><div class="card-body txt"><input type="hidden" name="name" data-sort="' + data[i].uid + '" value="' + j + '">' + data[i].name +
                              '<div><br>' +
                              '<a href="javascript:void(0);" class="btn btn_edit btn_pmk btn-sm item_edit" data-uid="' + data[i].uid + '" data-code="' + data[i].code + '" data-name="' + data[i].name + '" data-description="' + data[i].description + '"  data-groupprocessuid="' + data[i].groupprocessuid + '" data-counter="' + data[i].counter + '" data-active="' + data[i].active + '" data-startdate="' + data[i].start_date + '" data-enddate="' + data[i].end_date + '" style="font-family:cs_prajad_b;font-size:18px;font-weight:300;"><i class="fa fa-edit" aria-hidden="true"></i> แก้ไข</a>' + '' +
      ...
      function show_payor() {
         $.ajax({
            type: 'ajax',
            cache: false,
            url: ajaxURL + "/payor_data"+`?_${Date.now()}`,
            async: false,
            dataType: 'json',
            success: function(data) {
               var html = '';
      ...
      function show_patient_type_add(id) {
         $.ajax({
            type: 'ajax',
            cache: false,
            url: ajaxURL + "/patient_type_data"+`?_${Date.now()}`,
            async: false,
            dataType: 'json',
            success: function(data) {
               var html = '';
      ...
      $('#groupprocess_list').on('click', '.item_edit', function() {
         ...
         var end_datee = $(this).data('enddate');
         ...
         $('#timestart1').val(start_date);
         $('#timeend1').val(end_datee);

         // $(`.active`).attr("checked", false);
         // $(`.active[value="${active}"]`).trigger('click');
         // $(`#active_edit`).attr("checked", false);
         // $(`#active_edit[value="${active}"]`).trigger('click');
         // $(`#active_edit[value="${active}"]`).attr("checked", true);
         //$(`#active_edit[value="${active}"]`).attr("checked",true);
      ...
      $('#btn_update_edit').click(async function(e) {
         ...
         if(failed){
            await alert("หมวดหมู่คิวที่ท่านเลือกนั้นมีการเลือกไว้แล้ว(ของหมวดหมู่นี้)");
         }
         var uid_edit = $('#uid_edit').val();
         // alert(uid_edit);
         if(true){//if (failed == false) {
            await $.ajax({
