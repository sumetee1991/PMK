<script>
    ...
    var topicMap = {
        ...
        "management_visit_report":"kiosk_ทำประวัติ.เปิดสิทธิ_คัดกรอง",
    ...
    $(document).ready(function() {
        ...
        */
        //New Form
        $(document).on('submit','#report_form',function(e){
            let formURL = "<?= APPURLSETUP . '/Test_pm/report_topic'; ?>";
            let formData = $(this).serialize();
            $('#show_data').html('');
            $('#loading').css('display', 'block');
            $.post(formURL,formData)
                .done( (data) => {
                    ...
                    $('#show_data').html(data.html);
                    $(`#${data.table}`).dataTable({
                        ajax: `<?= APPURLSETUP . '/Test_pm/report_data'; ?>/${$('#topic').val()}/${$('#date_from').val()}/${$('#date_to').val()}`,
                        deferRender: true,
                        dom: 'fBrtip',
                        buttons: [
                            'copy', 'csv', 'excel', 'pdf', 'print'
