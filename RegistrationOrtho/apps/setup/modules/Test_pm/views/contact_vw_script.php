<script>
   ...
   $(document).ready(function() {
               ...
               for (i = 0; i < data.length; i++) {
                  if(data[i].name == "ทำประวัติ"){
                     html += '<div class="col-lg-4 col-sm-6 col-12 mb-2"><div class="card  shadow h-100  bg-light"><div class="card-body"><input type="hidden" value="' + j + '" data-sort="' + data[i].uid + '">' +
                        ...
                        '<div>' +
                        '<a href="javascript:void(0);" class="btn btn_pmk btn-sm item_edit btn_edit" data-uid="' + data[i].uid + '" data-product-name="' + data[i].name + '" data-allcounter="'+data[i].allcounter+'"><i class="fa fa-edit" aria-hidden="true"></i> แก้ไข</a>' + ' ' +
                        '</div>' +
                        '</div></div></div>';
                     j++;
                  }
      ...
      $('#show_data').on('click', '.item_edit', function() {
         $('#header_txt').html('ติดต่อช่องบริการ ใบนำทาง');
      ...
      $('#btn_update').click(function(e) {
         ...
         /*
         if ($('#allcounter').val() == "") {
            $('#validate-allcounter').html('กรุณากรอกการติดต่อช่องบริการใบนำทาง');
         } else {
            var formData = new FormData($('#submit')[0]);
            var uid = $('#uid').val();
            $.ajax({
