...
class Test_pm extends MY_Controller
{
   ...
   public function kiosk_data()
   {
      $this->load->model('Counter');
      $data = $this->Counter->get_kiosk_data();
      echo json_encode($data);
   }
   ...
   public function update_counter($id)
   {
      ...
      $data = array(
         ...
         'counterno' => $this->input->post('counterno'),
   ...
   public function add_queue_category()
   {
      ...
      $data = array(
         ...
         'start_date' => $this->input->post('timestart'),
         'end_date' => $this->input->post('timeend'),
         // 'active'=>$this->input->post('active'),
         // 'cwhen'=>date('Y-m-d h:i:s'),
      );
   ...
   public function add_payor()
   {
      ...
      $data = array(
         ...
         'active' => $this->input->post('active'),
         'counter' => json_encode($this->input->post('counter')),
         'cuser' => '1',
         'cwhen' => date('Y-m-d h:i:s'),
         'mwhen' => date('Y-m-d h:i:s'),
         'muser' => '1',
   ...
   public function add_kiosk()
   {
      $this->load->model('Counter');
      $data = array(
         'kiosklocation_id' => $this->input->post('code'),
         'queuelocation_id' => '2',
         'active' => 'Y',
      );
      // var_dump($data);
      // exit();
      $result = $this->Counter->add_kiosk($data);
      echo json_decode($result);
   }
   ...
   public function update_kiosk($id)
   {
      $data = array(
         'kiosklocation_id' => $this->input->post('code'),
      );
      $this->db->where('uid', $id);
      $result = $this->db->update('tb_kiosklocation', $data);
      $msg['success'] = false;
      if ($result) {
         $msg['success'] = true;
      }
   ...
   public function delete_kiosk()
   {
      $product_code = $this->input->post('product_code');
      $this->db->where('uid', $product_code);
      $result = $this->db->delete('tb_kiosklocation');
   ...
   public function update_building($id)
   {
      ...
      for ($i = 0; $i < $floor; $i++) {
         ...
         $data_floor = array(
            'buildinguid' => $id,
            'floor_number' => $_POST['floor'][$i]['floor_number'],
            'floor_name' => $_POST['floor'][$i]['floor_name'],
            'active' => 'Y',
            'cwhen' => date('Y-m-d h:i:s'),
            'mwhen' => date('Y-m-d h:i:s'),
            'muser' => '1',
   ...
   public function update_clinic($id)
   {
      ...
      $this->db->where('uid', $id);
      $result = $this->db->update('tb_walkinlimit', $data);
      $msg['success'] = false;
      if ($result) {
         $msg['success'] = true;
      }
   ...
   public function get_floor_building()
   {
      ...
      if (count($building) > 0) {
         ...
         foreach ($building as $building) {
            $pro_select_box .= "<input type='checkbox' id=chk" . $i . " name='remove_floor[remove_floor][]' value=" . $building->uid . " style='display:none;'><tr  id='row_floor$i'><td>" . 'ชั้น : ' . $building->floor_number . ($building->floor_name?'| ชื่อชั้น : '.$building->floor_name:'') . '</td><td><button class="btn_pmk btn_remove_floor" type="button"  id=' . $i . ' data-uid="' . $i . '">-</button></td></tr>';
            $i++;
         }
   ...
   public function report($topic = NULL,$date_from = NULL,$date_to = NULL){
      $this->load->module('Template_Module');
   ...
   public function report_data($topic,$date_from,$date_to){
      $this->load->model('Counter');
      switch($topic){
         case 0:
            $Data = $this->Counter->getKioskReport_spec($date_from,$date_to);
            break;
         case 1:
            $Data = $this->Counter->getVisitReport_spec($date_from,$date_to);
            break;
         case 2:
            $Data = $this->Counter->getNewHNReport_spec($date_from,$date_to);
            break;
         case 3:
            $Data = $this->Counter->getRegisterReport_spec($date_from,$date_to);
            break;
      }
   ...
   public function statistic(){
      ...
      $Template = array(
         ...
         'Site_Title' => 'test monk',
         'Content' => array(
            'test1' => 'Statistic/statistic_vw',
         ),
         'Script' => array(
            'testscript' => 'Statistic/statistic_vw_script',
         ),
         'css' => array(
            'bootstrap4.3.1' => 'bootstrap/css/bootstrap.min.css',
            'bootstrap-datepicker' => 'bootstrap-datepicker/css/bootstrap-datepicker.min.css',
   ...
   public function statistic_ajax(){
      ...
      switch($Input['topic']){
         case 0:
            $views['data'] = $this->Counter->getKioskStat($Input['date_select']);
            $output['stat'] = $views['data'];
            $output['html'] = $this->load->view('Statistic/ajax/kiosk',$views,TRUE);
            break;
         case 1:
            $views['data'] = $this->Counter->getVisitStat($Input['date_select']);
            $output['stat'] = $views['data'];
            $output['html'] = $this->load->view('Statistic/ajax/visit',$views,TRUE);
            break;
         case 2:
            $views['data'] = $this->Counter->getNewHNStat($Input['date_select']);
            $output['stat'] = $views['data'];
            $output['html'] = $this->load->view('Statistic/ajax/newhn',$views,TRUE);
            break;
         case 3:
            $views['data'] = $this->Counter->getRegisterStat($Input['date_select']);
            $output['stat'] = $views['data'];
            $output['html'] = $this->load->view('Statistic/ajax/register',$views,TRUE);
            break;
         default:
            $output['html'] = "Please Select Valid Topic";
            break;
