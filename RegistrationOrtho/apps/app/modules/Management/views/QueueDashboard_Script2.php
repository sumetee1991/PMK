<script type="text/javascript">
	...
	function refreshQueue(GroupprocessUID,Counter,QueueCategory,DOMELE,Table,callback){
		...
		var requestURL = base_path_url + 'ManagementViews2' + '/' + GroupprocessUID + '/' + strQueueCate;
	...
	$(document).ready(function() {
      ...
      $('#menu').html('<li><img style="width: 20px" src="<?=base_url('static/Icon');?>/3.png" alt=""><a class="sidenav_url" href="./AltDashboardlocation2"><button>คิวทำประวัติและเปิดสิทธิ</button></a></li>'+
         '<li><img style="width: 26px" src="<?=base_url('static/Icon');?>/1.png" alt=""><a class="sidenav_url" href="./Dashboardlocation2"><button>คัดกรอง</button></a></li>'+
         '<li id="SetupBtnxx"><img style="width: 20px" src="<?=base_url('static/Icon');?>/4.png" alt=""><a href="https://dashqueue.pmk.local/RegistrationOrtho/management/Management/Setup" ><button>ตั้งค่าเคาน์เตอร์</button></a></li>');
      ...
      localStorage.setItem = function(key, value) {
       ...
       updateWaitingQueue = setInterval(function() {
		...
		$(document).on("click", dataAttribute['Full']['Call'], function(e) {
         ...
         var queuelocation= localStorage.getItem('queuelocation');
...
// alert(postData);
        ...
        postAJAX(PostDataURL, postData, function() {
           ...
           CurlData = {
             ...
             'queuelocation':localStorage.getItem('queuelocation'),
          };
       ...
       if (!$(targetButton).hasClass('active')) {
		...
		$(document).on("click", dataAttribute['Full']['CompleteAll'], function(e) {
			...
			var complete9 = base_path_url + Management_CompleteURL + '23';
			postData = {
				'patientdetailuid': targetPatientUID,
				'queueno': targetQueueNumber,
				'counter': localStorage.getItem('Counter'),
			...
			var complete10 = base_path_url + Management_CompleteURL + '24';
			postData = {
				'patientdetailuid': targetPatientUID,
				'queueno': targetQueueNumber,
				'counter': localStorage.getItem('Counter'),
