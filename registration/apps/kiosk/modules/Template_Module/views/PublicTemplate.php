...
<head>
	...
	<?php
	$dataPt = json_decode(json_encode($this->session->userdata('SessionMain'), true), true);
	$idcard = '';
	$prename = '';
	$forename = '';
	$surname = '';
	...
	if ($dataPt) {
		...
		$idcard = (!isset($dataPt['UserInfo']) || $dataPt['UserInfo']['data'][0]['idcard'] == '') ? '' : $dataPt['UserInfo']['data'][0]['idcard'];
		$prename = (!isset($dataPt['UserInfo']) || $dataPt['UserInfo']['data'][0]['prefixname_th'] == '') ? '' : $dataPt['UserInfo']['data'][0]['prefixname_th'];
		$forename = (!isset($dataPt['UserInfo']) || $dataPt['UserInfo']['data'][0]['firstname_th'] == '') ? '' : $dataPt['UserInfo']['data'][0]['firstname_th'];
		$surname = (!isset($dataPt['UserInfo']) || $dataPt['UserInfo']['data'][0]['lastname_th'] == '') ? '' : $dataPt['UserInfo']['data'][0]['lastname_th'];
		$hn = (!isset($dataPt['UserInfo']) || @$dataPt['UserInfo']['data'][0]['hnno'] == '' || @$dataPt['UserInfo']['data'][0]['hnno'] == null) ? '-' : $dataPt['UserInfo']['data'][0]['hnno'];
		$birth = (!isset($dataPt['UserInfo']) || $dataPt['UserInfo']['data'][0]['issuedate'] == '') ? '-' : $dataPt['UserInfo']['data'][0]['issuedate'];
		$citizenid_xxx = (!isset($dataPt['UserInfo']) || @$dataPt['UserInfo']['data'][0]['citizenid_xxx'] == '') ? '-' : @$dataPt['UserInfo']['data'][0]['citizenid_xxx'];
	}
	...
	<div class='main_container'>
		...
		<div class="row no-gutters form-group center" style="box-shadow: 0px 3px 8px 1px #e1e0dc; border-radius: 4px 4px 4px 60px; padding:14px 30px 30px 30px ">
			...
			<div class="chk_img_user" style="text-align: center;">
				<span id="imgavatar"><?= single_img('kiosk/resources/images/AnonymousAvatar.png', array('class' => '_img_user')) ?></span>
			</div>
