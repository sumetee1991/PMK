	...
	$(document).ready(function() {
		...
		$.ajax({ //get persontype box
			...
			success: function(msg) {
				...
				$.each(msg, function(index, value) {
					...
					html += "<button class='col-3 in_checkbox chk_sty' style='outline:none;' value='" + value["code"] + "'>";
				...
				if (typeof get_from_appoint['PersonType']['PersonType'] != 'undefined') {
					$(".in_checkbox[value='" + get_from_appoint['PersonType']['PersonType'] + "']").find('div').find('.d2_lb1').css({
						'background-color': '#78b3a3',
						'color': 'white'
					}).attr('active', 'now');
					$(".in_checkbox[value='" + get_from_appoint['PersonType']['PersonType'] + "']").trigger('click');
				}
			}
		});
		...
		function midSocketkiosk_hn() {
			midSocket.on('kiosk_hn', (data) => { // เจอ hn หรือ ไม่เจอ ก็จะเข้าที่นี้
				...
				$.ajax({ //create session
					url: UrlBaseCont + '/StepProcessing/CreateSesPtShow',
					type: "post",
					async: false,
					dataType: "json",
					data: {
					...
					success: function(msg) {
						console.log(msg, 'ok Ptshow');
					},
					error: function(msg) {
						console.log(msg, 'error Ptshow');
					}
				});
