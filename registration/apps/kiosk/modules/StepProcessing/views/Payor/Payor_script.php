	...
	$(document).ready(function() {
		...
		$.ajax({
			...
			success: function(msg) {
				...
				if (typeof session_main['PayorActive']['PayorActive'] != 'undefined') {
					
					$(".btn_payor[value='" + session_main['PayorActive']['PayorActive'] + "']").find('div').find('.d2_lb1').css({
						'background-color': '#78b3a3',
						'color': 'white'
					}).attr('active', 'now');
					$(".btn_payor[value='" + session_main['PayorActive']['PayorActive'] + "']").attr('active_click', 'active').trigger('click');
		...
		try {
			...
			var idcard_sess = date_ses['UserInfo']['data'][0]['idcard'];

			var check_scan = date_ses['UserInfo']['scan'];


			var check_patient_type = date_ses['UserInfo']['worklistgroup'];

			var worklistgroup = date_ses['UserInfo']['worklistgroup'];
		...
		$("#conf_yes").click(function() {
			...
			if (check_patient_type == '3') { //ผู้ป่วยใหม่ ไม่ให้เปิดสิทธิอัตโนมัติ
				...
				if ($(".btn_payor[active_click='active']").attr('check_payor') == 'Y' && $(".btn_payor[active_click='active']").attr('payor_auto') != '') { // เก่า/นัด เลือกอัตโนมัติ click payor ที่เป็น เปิดสิทธิอัตโนมัติ
				...
				// 	// $("#queue_next").trigger('click');
				// } 
				else if ($(".btn_payor[active_click='active']").attr('check_payor') == 'N' || $(".btn_payor[active_click='active']").attr('check_payor') == 'Y' && worklistgroup != '3') { // [ทั่วไป คนเก่าเท่านั้น]  เก่า ปกติ กรณีทั่วไป หรือ y ที่เชคไม่ผ่านเงื่อนไข อัตโนมัติ
					$.each(btn_payor_active, function(index, value) {
						if (last_payor != value) {
							conf_yes();
						} else if (last_payor == value) {
							openqueue_success();
							$("#queue_next").trigger('click');
						}
					});
				} else if ($(".btn_payor[active_click='active']").attr('check_payor') == 'N' || $(".btn_payor[active_click='active']").attr('check_payor') == 'Y') {
					//alert('ปกติ');
					conf_yes();
				}
			} //else
