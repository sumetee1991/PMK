<script type="text/javascript">
	...
	$(document).ready(function() {
		$('#setupTitle').append('<br>คิวทำประวัติ');
		$('#setupTitle_Alert').append('<br>คิวทำประวัติ');
		...
		// var PostDataURL1 = base_path_url + 'alluser' ;
		// postData1 = {
		// 	'patientdetailuid': 1,
		// };
		// postAJAX(PostDataURL1, postData1);

		/*Location Queuecategory*/
		var RequestData = base_path_url + 'locationAvailable_JSON' + '/' + const_groupprocessid;
		...
		initQueue(RequestData, localStorage.getItem('Counter'), JSON.parse(localStorage.getItem('locatedata_' + const_groupprocessid))['QueueCate'], manageTable, function() {
			...
			midSocket.on('update_queue_action', function(data) {
				console.log("TEST DATA UPDATE QUEUE")
			...
			midSocket.on('remove_queue', function(data) {
				...
				if( Found ){
					...
					var getQueueURL = base_path_url+'GetQueueInfo_NewHN_Queueno/'+targetQueueNumber;
			...
			midSocket.on('manage_getqueue_puid', function(data) {
				...
				// 	var getPUIDInfoURL = base_path_url + 'GetQueueInfo_NewHN/' + targetPUID;
			...
			midSocket.on('update_action_call', function(data) {
				...
				// 	updateDTCell(manageTable, $(targetButton).parents('tr'), tableColumns['Status'], WorklistDescription);
				// });
		...
		$(document).on("click", dataAttribute['Full']['Cancel'], function(e) {
			...
			$('*' + dataAttribute['Semi']['Call'] + '="' + targetID + '"]').attr("disabled", true);
			$('*' + dataAttribute['Semi']['Hold'] + '="' + targetID + '"]').attr("disabled", true);
			$('*' + dataAttribute['Semi']['Note'] + '="' + targetID + '"]').attr("disabled", true);
			$('*' + dataAttribute['Semi']['Complete'] + '="' + targetID + '"]').attr("disabled", true);
